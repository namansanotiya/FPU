// ============================================================
// Testbench : tb_alu_rca_8bit
// Description : Simulation testbench for 8-bit RCA ALU
// ============================================================
`timescale 1ns / 1ps

module tb_alu_rca_8bit;
    ...
    initial begin
        $display("=== 8-bit RCA ALU Testbench ===");
        $display("%-6s %-5s %-5s | %-8s %-8s | %-5s %-5s %-5s | Status",
                 "OP", "A", "B", "result", "exp", "Cout", "Zero", "OVF");
        $display("-"*70);

        // ---- ADD ----
        A=8'd10;  B=8'd20;  opcode=3'b000; #10; print_result(A,B,opcode,8'd30,  1'b0);
        A=8'd200; B=8'd100; opcode=3'b000; #10; print_result(A,B,opcode,8'd44,  1'b1); // overflow carry
        A=8'd0;   B=8'd0;   opcode=3'b000; #10; print_result(A,B,opcode,8'd0,   1'b0); // zero
        A=8'd255; B=8'd1;   opcode=3'b000; #10; print_result(A,B,opcode,8'd0,   1'b1); // carry out

        // ---- SUB ----
        A=8'd50;  B=8'd30;  opcode=3'b001; #10; print_result(A,B,opcode,8'd20,  1'b1);
        A=8'd30;  B=8'd50;  opcode=3'b001; #10; print_result(A,B,opcode,8'd236, 1'b0); // borrow
        A=8'd100; B=8'd100; opcode=3'b001; #10; print_result(A,B,opcode,8'd0,   1'b1); // zero
    ...
    // Dump waveforms
    initial begin
        $dumpfile("tb_alu_rca_8bit.vcd");
        $dumpvars(0, tb_alu_rca_8bit);
    end
